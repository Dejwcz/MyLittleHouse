...
public sealed class MediaEndpointTests : IClassFixture<ApiWebApplicationFactory>
{
    private readonly ApiWebApplicationFactory _factory;
    private readonly HttpClient _client;

    public MediaEndpointTests(ApiWebApplicationFactory factory)
    {
        _factory = factory;
    ...
    [Fact]
    public async Task AddMedia_ListsAndGetsDetail()
    {
    ...
    public async Task UpdateMedia_UpdatesDescription()
    {
        ...
        var updateResponse = await ownerClient.PutAsJsonAsync(
            $"/media/{created.Id}",
            new UpdateMediaRequest("Updated"));
    ...
    [Fact]
    public async Task DeleteMedia_RemovesMedia()
    {
        var owner = await RegisterLoginAsync();
        var ownerClient = CreateAuthenticatedClient(owner.Token);
        var projectId = await CreateProjectAsync(ownerClient);
        ...
        var getResponse = await ownerClient.GetAsync($"/media/{created.Id}");
        Assert.Equal(HttpStatusCode.NotFound, getResponse.StatusCode);
    }

    [Fact]
    public async Task GetMediaUrl_ReturnsUrl()
    {
        var owner = await RegisterLoginAsync();
        var ownerClient = CreateAuthenticatedClient(owner.Token);
        var projectId = await CreateProjectAsync(ownerClient);
        ...
        var url = await urlResponse.Content.ReadFromJsonAsync<MediaUrlResponse>();
        Assert.NotNull(url);
